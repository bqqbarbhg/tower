...
object MeshOptimization {
  ...
  /**
    ...
    */
  def limitBoneAmountPerVertex(mesh: MeshResource, maxBonesPerVertex: Int): Unit = {
  ...
  def splitMeshByBoneAmount(mesh: MeshResource, maxBonesPerMesh: Int): Seq[MeshResource] = {
    ...
    val parts: ArrayBuffer[MeshResource] = for ((bones, indices) <- indexGroups) yield {
      val meshPart = new MeshResource(mesh.name + "." + meshPartIndex)
